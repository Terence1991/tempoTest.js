...
function renderCartPage(cart, products) {
	const page = $('.cart');
	page.addClass('visible');	
	...
	const theTemplateScript = $('#cart-template').html();
	const theTemplate = Handlebars.compile(theTemplateScript);

	const data = Object.keys(cart).reduce((arr, item) => {
		if (item !== 'total') {
			const product = products.filter((product) => product.id === +item)[0];
			arr.push({ ...product, qty: cart[item].qty})
		}
		return arr;
		}, []);

		console.log('fu',data, products );
		if (data.length) {
